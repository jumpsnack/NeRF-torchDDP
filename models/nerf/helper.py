...
def chunk_rays(inputs: torch.Tensor, chunksize: Optional[int] = 1024 * 8):
...
def sample_pdf_2(bins, weights, num_samples, det=False):
    ...
    inds = torch.searchsorted(cdf.detach(), u.contiguous(), right=True)
...
def get_ray_bundle(
        hwf, tform_cam2world: torch.Tensor
):
